  ...
  useEffect(() => {
    const fetchDetails = async () => {
      ...
      try {
        ...
        // If Vite serves index.html or source code instead of executing the serverless function
        if (text.includes("<!DOCTYPE html>") || text.includes("export default")) {
          console.warn("Local API not available. Fetching directly from GitHub...");
          const ghRes = await fetch(`https://api.github.com/repos/AyushBhardwaj-AdRuntime/${slug}`);
          if (!ghRes.ok) throw new Error("Failed to fetch repository details directly");
          const info = await ghRes.json();
          
          let readmeHtml = "<p>No README available.</p>";
          try {
            const readmeRes = await fetch(`https://api.github.com/repos/AyushBhardwaj-AdRuntime/${slug}/readme`, {
          ...
          let languages = {};
          try {
            const langRes = await fetch(`https://api.github.com/repos/AyushBhardwaj-AdRuntime/${slug}/languages`);
            if (langRes.ok) {
        ...
        if (!json || json.error) throw new Error(json?.error || "Failed to load project details");
        setData(json);
      } catch (err) {
